...
export default {
	...
	mounted () {
		...
		fetch('api/BASE_Limite_MSP.kml').then((file) => {
	...
	watch: {
		projetos (itens) {
			if (itens.length) {
				itens.map(item => {
					return fetch(`api/${item.kml}`).then((res) => {
						return res
					})
				}).filter(i => i.kml !== '18_PIUs_gestao_urbana_18CL.kml')
				itens.map(item => import(`../assets/shp/${item.shape}`)).filter(i => i.kml !== '18_PIUs_gestao_urbana_18CL.kml')
				this.initMapLayers(itens.filter(i => i.kml !== '18_PIUs_gestao_urbana_18CL.kml'))
			}
		},
